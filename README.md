...
Now let’s create a powerful side-by-side comparison that makes Simpson’s
Paradox crystal clear:

``` r
# Create the side-by-side comparison using patchwork
side_by_side_plot <- (plot1 | plot2) +
  plot_annotation(
    title = "Simpson's Paradox: The Same Data, Two Different Stories",
    subtitle = "Left: Overall relationship (positive) | Right: Species-specific relationships (negative)",
    caption = "This visualization demonstrates how aggregating data can completely reverse apparent relationships",
    theme = theme(
      plot.title = element_text(size = 18, face = "bold", hjust = 0.5),
      plot.subtitle = element_text(size = 14, hjust = 0.5, color = "gray40"),
      plot.caption = element_text(size = 12, hjust = 0, face = "italic", color = "gray60")
    )
  )
...
The left panel shows the misleading positive relationship when we ignore
species, while the right panel reveals the true negative relationships
within each species.
